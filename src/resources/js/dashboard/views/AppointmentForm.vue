...
const loadClients = async () => {
  try {
    const data = await clientsApi.getClients({ per_page: 1000 });
...
const loadAppointment = async () => {
  ...
  try {
    const appointment = await appointmentsApi.getAppointment(route.params.id);
    form.value = {
...
const saveAppointment = async () => {
  ...
  try {
    if (isEdit.value) {
      await appointmentsApi.updateAppointment(route.params.id, form.value);
    } else {
      await appointmentsApi.createAppointment(form.value);
    }
